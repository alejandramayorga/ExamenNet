...
import { CartItem } from '../carrito/cart.model';
...
export class ShopComponent implements OnInit {
  ...
  onCartItemDeleted(productData:{productId: number}) {
    const index = this.cartItems.findIndex( (elem: { id: number; }) => elem.id == productData.productId )
  ...
  updateCartTotal() {
    ...
    let total = 0;
    this.cartItems.map( (elem: { quantity: number; price: number; }) => total = total + elem.quantity*elem.price);
